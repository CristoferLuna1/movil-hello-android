...
interface TransactionDao {
    ...
    @Query("SELECT * FROM transactions ORDER BY date DESC") suspend fun getAll(): List<Transaction>
    ...
    @Query(
            "SELECT * FROM transactions WHERE title LIKE :searchQuery OR category LIKE :searchQuery ORDER BY date DESC"
    )
    ...
    @Query(
            "SELECT * FROM transactions WHERE userId = :userId AND monthYear = :monthYear ORDER BY date DESC"
    )
    ...
    @Query("SELECT * FROM transactions") suspend fun getAllOnce(): List<Transaction>
    ...
    @Query("DELETE FROM transactions WHERE id = :id") suspend fun deleteById(id: Int)

    @Query("DELETE FROM transactions WHERE firebaseId = :firebaseId")
